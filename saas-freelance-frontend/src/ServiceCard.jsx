...
export default function ServiceCard({ service, userType, onAccept, onCounterOffer, onRefuse, onClientApprove, onClientReject, onClick }) {
  ...
  // Impede que o clique nos botões abra o modal de detalhes
  const stopProp = (e) => e.stopPropagation();

  return (
    // ADICIONADO: onClick no container principal e cursor-pointer
    <div 
      onClick={onClick}
    ...
    >
      ...
      {/* Botão X (Prestador) - Com stopProp para não abrir modal ao fechar */}
      {userType === 'PRESTADOR' && (
        <button 
          onClick={(e) => { stopProp(e); onRefuse(service.id); }} 
          className="absolute top-6 right-6 text-gray-500 hover:text-red-400 transition z-10"
        >
          <XCircle size={24} />
        </button>
      )}
      ...
      <div className="pt-6 border-t border-white/10 mt-6">
        <div className="flex flex-col gap-4">
          ...
          {/* BOTÕES DE AÇÃO - Com stopProp para o clique ficar só aqui */}
          <div className="flex gap-3 w-full" onClick={stopProp}>
            {userType === 'PRESTADOR' && (
              <>
                <button onClick={() => onCounterOffer(service)} className="flex-1 py-3 bg-blue-600/20 hover:bg-blue-600 text-blue-300 hover:text-white rounded-xl transition border border-blue-500/30 flex justify-center items-center gap-2 font-bold text-sm">
                  <DollarSign size={18} /> Negociar
                </button>
                <button onClick={() => onAccept(service.id)} className="flex-[2] py-3 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold rounded-xl transition flex justify-center items-center gap-2 shadow-lg">
